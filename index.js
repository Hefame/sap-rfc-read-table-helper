const splitWhereClause = (where) => {

	let chunks = [];
	let lastIndexOfSpace = 0;
	let lastCutIndex = 0;

	for (let i = 0; i < where.length; i++) {
		if (where.charAt(i) === ' ') lastIndexOfSpace = i

		if (i - lastCutIndex >= 72) {
			if (!lastIndexOfSpace) lastIndexOfSpace = i
			chunks.push(where.substring(lastCutIndex, lastIndexOfSpace));
			lastCutIndex = lastIndexOfSpace + 1;
		}
	}

	chunks.push(where.substring(lastCutIndex, where.length));
	return chunks;
...
const rfcReadTable = async (sapClient, { table, fields, where, skip = 0, limit = 0, fieldNameMap = null, rowMapFn = null }) => {
	...
	const query = {
		...
		ROWSKIPS: skip,
		ROWCOUNT: limit,
		OPTIONS: where?.length ? splitWhereClause(where) : [],
